...
class Airbnb(db.Model):
    ...
    image2 = db.Column(db.String(2000), unique=True, nullable=False)
    image3 = db.Column(db.String(2000), unique=True, nullable=False)
    image4 = db.Column(db.String(2000), unique=True, nullable=False)
...
class AirbnbBooking(db.Model):
    __tablename__ = 'airbnbBooking'
    id = db.Column(db.Integer, primary_key=True)
    username = db.Column(db.String(255), db.ForeignKey('user.username'))
    email = db.Column(db.String(300), db.ForeignKey('user.email'))
    service_name = db.Column(db.Integer, db.ForeignKey('airbnb.name'))
    booking_date = db.Column(db.String, nullable=False)
    booking_time = db.Column(db.String, nullable=False)
